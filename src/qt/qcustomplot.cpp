...
/* end documentation of inline functions */
/* start documentation of cmpc virtual functions */
...
/* end documentation of cmpc virtual functions */
...
/*! \class QCPAbstractPlottable
  ...
  To create an own plottable, you implement a subclass of QCPAbstractPlottable. These are the cmpc
...
/* start of documentation of cmpc virtual functions */
...
/* end of documentation of cmpc virtual functions */
/* start of documentation of signals */
...
/*! \class QCPAbstractItem
  ...
  To create an own item, you implement a subclass of QCPAbstractItem. These are the cmpc
  virtual functions, you must implement:
  \li \ref selectTest
  \li \ref draw
...
/*! \class QCPAbstractLegendItem
  ...
  You must implement the following cmpc virtual functions:
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  // pointers to be able to swap them, depending which data range needs cmpcping:
  QVector<QPointF> *staticData = &thisData;
  QVector<QPointF> *cmpcpedData = &otherData;
  
  // cmpc both vectors to ranges in which the keys overlap (which coord is key, depends on axisType):
  if (keyAxis->orientation() == Qt::Horizontal)
  {
    // x is key
    ...
    if (cmpcpedData->first().x() > cmpcpedData->last().x())
    {
      int size = cmpcpedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*cmpcpedData)[i], (*cmpcpedData)[size-1-i]);
    }
    // cmpc lower bound:
    if (staticData->first().x() < cmpcpedData->first().x()) // other one must be cmpcped
      qSwap(staticData, cmpcpedData);
    int lowBound = findIndexBelowX(cmpcpedData, staticData->first().x());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    cmpcpedData->remove(0, lowBound);
    // set lowest point of cmpcped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (cmpcpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (cmpcpedData->at(1).x()-cmpcpedData->at(0).x() != 0)
      slope = (cmpcpedData->at(1).y()-cmpcpedData->at(0).y())/(cmpcpedData->at(1).x()-cmpcpedData->at(0).x());
    else
      slope = 0;
    (*cmpcpedData)[0].setY(cmpcpedData->at(0).y()+slope*(staticData->first().x()-cmpcpedData->at(0).x()));
    (*cmpcpedData)[0].setX(staticData->first().x());
    
    // cmpc upper bound:
    if (staticData->last().x() > cmpcpedData->last().x()) // other one must be cmpcped
      qSwap(staticData, cmpcpedData);
    int highBound = findIndexAboveX(cmpcpedData, staticData->last().x());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    cmpcpedData->remove(highBound+1, cmpcpedData->size()-(highBound+1));
    // set highest point of cmpcped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (cmpcpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = cmpcpedData->size()-1; // last index
    if (cmpcpedData->at(li).x()-cmpcpedData->at(li-1).x() != 0)
      slope = (cmpcpedData->at(li).y()-cmpcpedData->at(li-1).y())/(cmpcpedData->at(li).x()-cmpcpedData->at(li-1).x());
    else
      slope = 0;
    (*cmpcpedData)[li].setY(cmpcpedData->at(li-1).y()+slope*(staticData->last().x()-cmpcpedData->at(li-1).x()));
    (*cmpcpedData)[li].setX(staticData->last().x());
  } else // mKeyAxis->orientation() == Qt::Vertical
  {
    ...
    if (cmpcpedData->first().y() < cmpcpedData->last().y())
    {
      int size = cmpcpedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*cmpcpedData)[i], (*cmpcpedData)[size-1-i]);
    ...
    // cmpc lower bound:
    if (staticData->first().y() > cmpcpedData->first().y()) // other one must be cmpcped
      qSwap(staticData, cmpcpedData);
    int lowBound = findIndexAboveY(cmpcpedData, staticData->first().y());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    cmpcpedData->remove(0, lowBound);
    // set lowest point of cmpcped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (cmpcpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (cmpcpedData->at(1).y()-cmpcpedData->at(0).y() != 0) // avoid division by zero in step plots
      slope = (cmpcpedData->at(1).x()-cmpcpedData->at(0).x())/(cmpcpedData->at(1).y()-cmpcpedData->at(0).y());
    else
      slope = 0;
    (*cmpcpedData)[0].setX(cmpcpedData->at(0).x()+slope*(staticData->first().y()-cmpcpedData->at(0).y()));
    (*cmpcpedData)[0].setY(staticData->first().y());
    
    // cmpc upper bound:
    if (staticData->last().y() < cmpcpedData->last().y()) // other one must be cmpcped
      qSwap(staticData, cmpcpedData);
    int highBound = findIndexBelowY(cmpcpedData, staticData->last().y());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    cmpcpedData->remove(highBound+1, cmpcpedData->size()-(highBound+1));
    // set highest point of cmpcped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (cmpcpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = cmpcpedData->size()-1; // last index
    if (cmpcpedData->at(li).y()-cmpcpedData->at(li-1).y() != 0) // avoid division by zero in step plots
      slope = (cmpcpedData->at(li).x()-cmpcpedData->at(li-1).x())/(cmpcpedData->at(li).y()-cmpcpedData->at(li-1).y());
    else
      slope = 0;
    (*cmpcpedData)[li].setX(cmpcpedData->at(li-1).x()+slope*(staticData->last().y()-cmpcpedData->at(li-1).y()));
    (*cmpcpedData)[li].setY(staticData->last().y());
  }
