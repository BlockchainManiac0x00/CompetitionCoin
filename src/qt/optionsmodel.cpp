...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeCompetitionCoinAmount"))
        settings.setValue("nAnonymizeCompetitionCoinAmount", 1000);
    nAnonymizeCompetitionCoinAmount = settings.value("nAnonymizeCompetitionCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeCompetitionCoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeCompetitionCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeCompetitionCoinAmount:
            return QVariant(nAnonymizeCompetitionCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeCompetitionCoinAmount:
            nAnonymizeCompetitionCoinAmount = value.toInt();
            settings.setValue("nAnonymizeCompetitionCoinAmount", nAnonymizeCompetitionCoinAmount);
            emit AnonymizeCompetitionCoinAmountChanged(nAnonymizeCompetitionCoinAmount);
            break;
        default:
            break;
        }
