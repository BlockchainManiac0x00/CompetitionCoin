...
{
    // Convert cmpc:// to cmpc:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("cmpc://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "cmpc:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CompetitionCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for CompetitionCoin.lnk
...
{
    return GetAutostartDir() / "cmpc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a cmpc.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=CompetitionCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the cmpc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add cmpc app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CompetitionCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  cmpc-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("CompetitionCoin-Qt"));
