...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeCompetitionCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCompetitionCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeCompetitionCoinAmount = strAnonymizeCompetitionCoinAmount.remove(strAnonymizeCompetitionCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCompetitionCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeCompetitionCoinAmount*COIN) nMaxToAnonymize = nAnonymizeCompetitionCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeCompetitionCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeCompetitionCoinAmount));
        strAnonymizeCompetitionCoinAmount = strAnonymizeCompetitionCoinAmount.remove(strAnonymizeCompetitionCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCompetitionCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeCompetitionCoinAmount == 0){
