...
int nAnonymizeCompetitionCoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CompetitionCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CompetitionCoin
    // Mac: ~/Library/Application Support/CompetitionCoin
    // Unix: ~/.competitioncoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CompetitionCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "CompetitionCoin";
#else
    // Unix
    return pathRet / ".competitioncoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "competitioncoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "competitioncoind.pid"));
