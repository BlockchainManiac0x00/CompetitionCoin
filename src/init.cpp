...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: cmpc.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: cmpcd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizecmpcamount=<n> " + _("Keep N CompetitionCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CompetitionCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CompetitionCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CompetitionCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "CompetitionCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CompetitionCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart CompetitionCoin to complete") << "\n";
    ...
    nAnonymizeCompetitionCoinAmount = GetArg("-anonymizecmpcamount", 0);
    if(nAnonymizeCompetitionCoinAmount > 999999) nAnonymizeCompetitionCoinAmount = 999999;
    if(nAnonymizeCompetitionCoinAmount < 2) nAnonymizeCompetitionCoinAmount = 2;
    ...
    LogPrintf("Anonymize CompetitionCoin Amount %d\n", nAnonymizeCompetitionCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1CMPC+1000 == (.1CMPC+100)*10
       10CMPC+10000 == (1CMPC+1000)*10
    */
